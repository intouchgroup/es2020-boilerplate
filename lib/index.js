...
const generateConfig = ({ entryFile, scriptOutputFilename, styleOutputFilename, devMode, nodeModulesToBabel, plugins, ignoredOutputFiles, polyfill, skipLinting, skipStyles }) => ({
    ...
    plugins: [
        skipLinting ? undefined : pluginLintStyles(),
        skipStyles ? undefined : pluginExtractStyles(styleOutputFilename),
        skipStyles ? undefined : pluginOptimizeStyles(devMode),
    ...
    try {
        ...
        return Object.entries(entries).map(([ entryKey, entryValue ]) => {
            if (typeof entryValue === 'string') {
                ...
                const { file, plugins = [], polyfill = true, skipLinting = false, skipStyles = false } = entryValue;
