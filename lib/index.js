...
const pluginLintStyles = () => new PluginStyleLint({ fix: true, allowEmptyInput: true });
...
const generateConfig = ({ entryFile, scriptOutputFilename, styleOutputFilename, devMode, nodeModulesToBabel, plugins, ignoredOutputFiles, polyfill, skipLinting }) => ({
...
const createWebpackConfig = ({ argv, entries, nodeModulesToBabel, env }) => {
    try {
        if (env) {
